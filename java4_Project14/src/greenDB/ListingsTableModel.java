...
class ListingsTableModel extends AbstractTableModel {
     ...
     * a single column for output. This method always returns 4 for Last Name, First
     * Name, Phone Number, and ID.
     * <pre>
     * PRE: True
     * POST: The number 4 is returned.
     * </pre>
     * @return the number 4, for the four output columns Last Name, First Name, Phone Number, and ID.
     */
     ...
     * <pre>
     * PRE:  Column is assigned and 0 >= column <= 3.
    ...
    public String getColumnName(int column) {
        try {
            ...
            // Since getColumnCount always returns 4, we only look for first 4 columns in
            // the result set.
            if(colName.equals("LAST_NAME"))
                ...
                return "Phone Number";
            else if(colName.equals("LISTINGS_ID"))
                return "ID";
     ...
     * is equal to 4 (the AREA_CODE), combine the AREA_CODE, PREFIX, and SUFFIX for that row and
     * return the combined string.
     * <pre>
     * PRE:  row and column are assigned and 0 >= column <= 3 and row is within range.
     * POST: The value in the result set at row and column is returned, or the combined
     *       phone number is returned if column = 4.
     * </pre>
    ...
    public Object getValueAt(int row, int column) {
        try {
            rs.absolute(row + 1);
            // for the 4th column in the results, combine all of the phone number fields for output
            if(column == 4)
                return "(" + rs.getObject(column + 4) + ") " + rs.getObject(column + 5) + "-" + rs.getObject(column + 6);
            else
