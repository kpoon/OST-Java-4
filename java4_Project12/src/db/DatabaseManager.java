...
public class DatabaseManager {
    ...
    private ResultSet resultSet, getDuplicate;										  // results from a database query
    ...
    public void addEntry (String name, String phoneNumber ){                       // adds an entry to the Phone Book
        try
        {
        	getDuplicate = statement.executeQuery("SELECT Name FROM PhoneBook WHERE Name LIKE" + "'%" + name + "%'");
        	int count = 0;

        	while (getDuplicate.next()) {
        	    ++count;
        	}
        	...
        	if (count == 0) {
        		...
        		System.out.println("Cannot insert record.  Name already exists.");
        	}     	
        }
        catch ( SQLException exception ) 
        {
